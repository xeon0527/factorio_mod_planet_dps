local function _research_update(e)
  ...
  --local force = game.forces["player"]
  --local tech = force.technologies
  -- if e.research.name == "dps-tech_advanced-apfsds-shell-enhancement-chance" then
  --   local recipes = force.recipes
  ...
  --   for i = 6, 10, 1 do
  --     for j = 0, 85, 5 do
  --       local ptyp = recipes["dps-ammo_apfsds-shell_"..i.."_"..j].prototype
  --       local probability_1 = 1.0 / (ptyp.main_product.shared_probability.max / 0.01)
  --       recipes["dps-ammo_apfsds-shell_"..i.."_"..j].productivity_bonus = probability_1 * level
  --     end
  --   end

  -- elseif e.research.name == "dps-tech_advanced-apfsds-shell-destruction-protection" then
  --   local recipes = force.recipes
  --   local level = e.research.level - 1
  --   if e.research.researched then
  --     level = level + 1
  ...
  --   for i = 6, 10, 1 do
  --     for j = 0, 85, 5 do
  --       recipes["dps-ammo_apfsds-shell_"..i.."_"..j].enabled = false
  --       recipes["dps-ammo_apfsds-shell_"..i.."_"..j].hidden_from_flow_stats = true
  --     end
  --   end

  --   if tech["dps-tech_advanced-apfsds-shell"].researched then
  --     for i = 6, 10, 1 do
  --       recipes["dps-ammo_apfsds-shell_"..i.."_"..(level*5)].enabled = true
  --       recipes["dps-ammo_apfsds-shell_"..i.."_"..(level*5)].hidden_from_flow_stats = false
  --     end
  --   end
  ...
  --       if recipe and util.string_starts_with(recipe.name, "dps-ammo_apfsds-shell_") then
